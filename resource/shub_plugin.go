...
func GetCredentials(configuartion string, prop *[]types.Property) error {
	...
	if err != nil {
		return errors.Wrap(err, "GetCredentials: configuration Unmarshal Failed")
	}
	return err
}
...
func GetSHUBPublicKey() []byte {
	...
	if err != nil {
		log.WithError(err).Info("GetSHUBPublicKey: Error in reading the shub pem file")
	}
	return pubKey
}

func createSignedTrustReport(createSignedTrustReport string) (string, error) {
	log.Trace("resource/sgx_plugin: createSignedTrustReport() Entering")
	defer log.Trace("resource/sgx_plugin: createSignedTrustReport() Leaving")
...
func processDataToPlugins(t1 types.Tenant, h1 []types.HostDetails, p1 []types.Plugin, db repository.SHUBDatabase) error {
	...
	for _, plugin := range p1 {
		...
		if value == "" {
			////report error
			return errors.New("processDataToPlugins: value of plugin provider is null")
		}
		///Get Plugin class name
		err := addCredentialToPlugin(t1, &plugin, db)
		if err != nil {
			return errors.Wrap(err, "processDataToPlugins: Couldn't add credentials for plugin")
		}
		if value == "Kubernetes" {
			var k1 plugins.Kubernetes
			err := k1.Pushdata(pData, plugin)
			if err != nil {
				log.WithError(err).Info("processDataToPlugins: got error while pushing the data to Kubernetes")
				return err
			}
			...
			if err != nil {
				log.WithError(err).Info("processDataToPlugins: got error while pushing the data to openstack")
				return err
			}

		} else {
			return errors.New("processDataToPlugins: plugin provider doesn't match")
		}
	}
	return nil
}
...
func addCredentialToPlugin(t1 types.Tenant, p1 *types.Plugin, db repository.SHUBDatabase) error {
	log.Trace("resource/processDataToPlugins: addCredentialToPlugin() Entering")
	defer log.Trace("resource/processDataToPlugins: addCredentialToPlugin() Leaving")
	...
	if err != nil {
		return errors.Wrap(err, "addCredentialToPlugin: didn't find any credential for plugin")
	}
	///Get all the credentials from db which will come as a string.
	...
	if err != nil {
		return errors.Wrap(err, "addCredentialToPlugin: couldn't get any credentials")
	}
	...
	log.Debug("addCredentialToPlugin: values of plugins after credentials: ", p1)
	return nil
}

func populateHostDetails(h1 *types.Host) (*types.HostDetails, error) {
	log.Trace("resource/processDataToPlugins: populateHostDetails() Entering")
	defer log.Trace("resource/processDataToPlugins: populateHostDetails() Leaving")
	...
	if err != nil {
		return nil, errors.Wrap(err, "populateHostDetails: Error parsing time string")
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "populateHostDetails: failed to marshal hostPlatformData to get trustReport")
	}
	trustReport := string(trustReportBytes)
	signedTrustReport, err := createSignedTrustReport(trustReport)
	if err != nil {
		return nil, errors.Wrap(err, "populateHostDetails: Failed to get signed trust report")
	}
