...
func (s Server) Run(c setup.Context) error {
	...
	defaultPort, err := c.GetenvInt("SHUB_PORT", "SGX Attestation Hub Service secure port")
	if err != nil {
		defaultPort = constants.DefaultHttpsPort
	}
	fs := flag.NewFlagSet("server", flag.ContinueOnError)

	fs.IntVar(&s.Config.Port, "port", defaultPort, "SGX Attestation Hub Service secure port")
